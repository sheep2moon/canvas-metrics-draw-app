export const drawElement = (context: CanvasRenderingContext2D, element: CanvasElement) => {
    ...
    if (element.type === "line") {
        ...
        const lineLenght = Math.sqrt(Math.pow(element.endPosition.x - element.startPosition.x, 2) + Math.pow(element.endPosition.y - element.startPosition.y, 2));

        context.font = "16px serif";
        context.fillText(lineLenght.toString(), centerX, centerY);
